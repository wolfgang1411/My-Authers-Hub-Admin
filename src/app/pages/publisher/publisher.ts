...
@Component({
  ...
  imports: [SharedModule, CommonModule, MatIconModule, MatButton, ListTable, RouterLink, MatIconButton],
...
displayedColumns: string[] =['serial', 'name', 'email', 'phonenumber', 'titles', 'authors', 'companyname' , 'actions'];
ngOnInit(): void {
 this.searchStr.pipe((debounceTime(400))
  ...
  this.publisherService.getPublishers().then(({items}) => {
    ...
    serial: idx + 1,
    name:publisher.user ? publisher.user.firstName + " " + publisher.user.lastName : '',
    email: publisher.email,
    phonenumber: publisher.phoneNumber,
    titles: publisher.titles && publisher.titles.length ? publisher.titles.length : 0,
    authors: publisher.authors && publisher.authors.length ? publisher.authors.length : 0,
