...
export class AddTitle implements OnInit, OnDestroy {
  ...
  isNewTitle = computed(() => this.titleId() === 0);
  currentStep = signal<
  ...
  addTitleForm = new FormGroup({
    ...
    authorRoyalties: new FormArray<FormGroup<{ authorId: FormControl<number | null>, percentage: FormControl<number | null> }>>([]),
  });
  ...
  async prefillForm(title: Partial<Title>) {
    ...
    this.addTitleForm.patchValue({
      ...
      titleDetails: {
        ...
        authorIds: title.authors?.map((authorEntry) => {
          const user = authorEntry.author.user;
          const fullName = user?.fullName || (user ? `${user.firstName || ''} ${user.lastName || ''}`.trim() : 'N/A');
          return {
            id: authorEntry.author.id,
            allowAuthorCopy: authorEntry.allowAuthorCopy,
            display_name: authorEntry.display_name,
            displayName: authorEntry.display_name,
            name: fullName,
            keepSame: authorEntry.display_name === fullName,
          };
        }),
        ...
        isEbookIsbnAutoGenerated: title?.isbnEbook?.startsWith('B'),
      ...
      authorPercentMap.forEach((percentage, authorId) => {
        this.addTitleForm.controls.authorRoyalties.push(
          new FormGroup({
            ...
            percentage: new FormControl<number | null>(percentage, [Validators.required, Validators.min(0), Validators.max(100)]),
          })
        );
      });
    }
      ...
      .subscribe(async (v) => {
        ...
        if (v !== PublishingType.ONLY_EBOOK && !fullCoverControl) {
          ...
          this.documentMediaArray().insert(
            3,
            await this.createMedia(TitleMediaType.FULL_COVER, true),
          );
    ...
    this.documentMediaArray()
      ...
      .subscribe((medias) => {
        ...
        if (interior && interior.noOfPages) {
          ...
          if (totalPagesControl.disabled) {
            totalPagesControl.enable({ emitEvent: false });
          }
        }
      });
  }
  ...
  private getPlatformMsp(platformName: string): number {
    const isEbook = this.platformService.getPlatformByName(platformName)?.isEbookPlatform;
    if (isEbook) {
