...
export class AddTitleDetails implements OnInit, OnDestroy {
  ...
  constructor() {
    effect(() => {
      ...
      const titleAuthor = this.title()?.authors;

      titleAuthor?.forEach(({ author }) => {
        if (options.find((opt) => opt.value === author.id)) {
          return;
        }

        options.push({
          label: (() => {
            const fullName = `${author.user?.firstName || ''} ${author.user?.lastName || ''} (${author.username})`;
            return author.user ? fullName : author.username;
          })(),
          value: author.id,
        });
      });
    ...
    effect(() => {
      ...
      const titlePublisher = this.title()?.publisher;
      if (
        titlePublisher &&
        !options.find((opt) => opt.value === titlePublisher.id)
      ) {
        options.push({
          label: titlePublisher.name,
          value: titlePublisher.id,
        });
      }
  ...
  onIsbnBlur(controlName: 'isbnPrint' | 'isbnEbook') {
    ...
    if (control?.value) {
      ...
      // Trigger API validation after formatting
      if (controlName === 'isbnPrint') {
        this.verifyIsbnPrint();
      } else if (controlName === 'isbnEbook') {
        this.verifyIsbnEbook();
      }
    } else {
  ...
  async verifyIsbnPrint(): Promise<void> {
    const isbnNumber = this.detailsGroup().controls.isbnPrint.value;
    ...
    try {
      ...
      if (result.verified) {
        ...
        this.detailsGroup().controls.isbnPrint.setErrors(null);
      } else {
        this.isbnVerified.set(false);
        this.detailsGroup().controls.isbnPrint.setErrors({
          invalidIsbn: true,
        });
      }
    } catch (err) {
  ...
  async verifyIsbnEbook(): Promise<void> {
    const isbnNumber = this.detailsGroup().controls.isbnEbook.value;
    if (!isbnNumber) return;

    // Don't verify auto-generated ISBNs as they are generated by the system
    if (
      this.detailsGroup().controls.isEbookIsbnAutoGenerated.value ||
      isbnNumber?.toLowerCase()?.startsWith('bcbl')
    ) {
      this.isbnEbookVerified.set(true);
      this.detailsGroup().controls.isbnEbook.setErrors(null);
      return;
    }
    ...
    try {
      ...
      if (result.verified) {
        ...
        this.detailsGroup().controls.isbnEbook.setErrors({
          invalidIsbn: true,
        });
      }
    } catch (err) {
    ...
    try {
      ...
      if (!validAuthors?.length) {
        Swal.fire({
          ...
          text: 'Please select at least one author.',
        });
        return;
      }
