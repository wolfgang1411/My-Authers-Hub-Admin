  ...
  async onTitleSubmit() {
    ...
    const basicData: TitleCreate = {
      ...
      launch_date: (() => {
        ...
        const dateControlValue: any = this.tempForm.controls.titleDetails.controls.launch_date.value;
        if (!dateControlValue) return undefined;

        // If it's a Date object (from date picker), format it
        if (dateControlValue instanceof Date && !isNaN(dateControlValue.getTime())) {
          return format(dateControlValue, 'yyyy-MM-dd');
        }

        // If it's already in YYYY-MM-DD format, return as is
        if (typeof dateControlValue === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(dateControlValue)) {
