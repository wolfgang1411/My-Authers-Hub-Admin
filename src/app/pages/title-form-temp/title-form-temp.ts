...
export class TitleFormTemp implements OnDestroy {
  ...
  /**
   * Uploads media files sequentially with progress tracking
   */
  private async uploadMediaWithProgress(
    ...
    // Use loadPromise to manage the loader, but update messages during upload
  ...
  async savePrintingDraft() {
    ...
    try {
      ...
      if (this.isRaisingTicket()) {
        ...
        // Include author copy permissions if any
        if (authorCopyPermissions.length > 0) {
          updateTicketData.authorCopyPermissions = authorCopyPermissions;
        }
      ...
      const createPrinting: PrintingCreate = {
        ...
        // Include author copy permissions if any
        ...(authorCopyPermissions.length > 0 && {
          authorCopyPermissions,
        }),
      };
  ...
  getActionButtonText(stepName?: string): string {
    ...
    switch (step) {
      ...
      case 'distribution':
        if (isRaisingTicket) {
          ...
          actionText =
            this.translateService.instant('uploadtitle') || 'Upload Title';
        }
        break;
    ...
    return navigationText ? `${actionText} and ${navigationText}` : actionText;
  }
