...
import { SharedPublisherProfile } from '../../interfaces/Publishers';
import { AngularSvgIconModule } from 'angular-svg-icon';

@Component({
  selector: 'app-shared-publisher-view',
  imports: [
    Back,
    SharedModule,
    MatTabsModule,
  ...
  templateUrl: './shared-publisher-view.html',
  styleUrl: './shared-publisher-view.css',
})
export class SharedPublisherView implements OnInit {
  id!: number;
  publisherData = signal<SharedPublisherProfile | null>(null);
  ...
  async fetchProfile() {
    try {
      ...
      this.error.set(null);
      const data = await this.publisherService.getSharedProfile(this.id);
      this.publisherData.set(data);
    } catch (error: any) {
      console.error('Error fetching shared publisher profile:', error);
      this.error.set(
        error?.error?.message || 'Failed to load shared publisher profile'
      );
    } finally {
  ...
  getLogoUrl(): string | null {
    const medias = this.publisherData()?.medias || [];
    const logo = medias.find((m) => m.type === 'LOGO');
    return logo ? logo.url : medias.length > 0 ? medias[0].url : null;
  }

  getPlatformIconUrl(platformIcon?: string, platformName?: string): string | null {
